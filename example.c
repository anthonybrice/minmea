...
int main(void)
{
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        case MINMEA_SENTENCE_GRS: {
            struct minmea_sentence_grs frame;
            if (minmea_parse_grs(&frame, line)) {
                printf(INDENT_SPACES "$xxGRS: time = %d:%d:%d.%d\n",
                       frame.time.hours, frame.time.minutes,
                       frame.time.seconds, frame.time.microseconds);
                for (int i = 0; i < 12; i += 4)
                    printf(INDENT_SPACES "$xxGRS: %f %f %f %f\n",
                           minmea_tofloat(&frame.residuals[i]),
                           minmea_tofloat(&frame.residuals[i+1]),
                           minmea_tofloat(&frame.residuals[i+2]),
                           minmea_tofloat(&frame.residuals[i+3]));
            }
        } break;
